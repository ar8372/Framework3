...
class predictor(OptunaOptimizer):
    ...
    def run_folds(self):
        ...
        temp_valid_predictions = pd.DataFrame.from_dict(oof_prediction, orient='index').reset_index()
        temp_valid_predictions.columns = [f"{self.locker['id_name']}", f"pred_l_{self.current_dict['current_level']}_e_{self.exp_no}"]
        my_folds[ f"pred_l_{self.current_dict['current_level']}_e_{self.exp_no}"] = temp_valid_predictions[f"pred_l_{self.current_dict['current_level']}_e_{self.exp_no}"]
        my_folds.to_csv(f"../models_{self.locker['comp_name']}/my_folds.csv", index=False)
        # save temp predictions
        test[f"pred_l_{self.current_dict['current_level']}_e_{self.exp_no}"] = stats.mode(np.column_stack(test_predictions), axis=1)[0]
        ...
        # -----------------------------update current dict
        self.current_dict["current_feature_no"] = self.current_dict["current_feature_no"] + 1
