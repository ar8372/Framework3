...
class OptunaOptimizer:
    def __init__(
        ...
        self.model_list = ["lgr","lir","xgbc","xgbr","cbc","mlpc", "rg", "ls","knnc", "dtc", "adbc", "gbmc" ,"hgbc", "lgbmc", "lgbmr", "rfc" , "k1" ]
    ...
    def get_params(self, trial):
            ...
            params = {
                "class_weight": trial.suggest_categorical(
                    ...
                    [
                        "balanced",
                        None,
                        {1: 1, 0: (sum(list(self.ytrain == 0)) / sum(list(self.ytrain == 1)))},
                    ],
                ),
        ...
        if model_name == "rg":
            params = {
                "alpha": trial.suggest_categorical("alpha", list(np.linspace(1,100,100))),
        ...
        if model_name == "dtc":
            params = {
                    ...
                    'min_samples_leaf': trial.suggest_categorical('min_samples_leaf',[5, 10, 15, 20,25]),
                    "min_samples_split": trial.suggest_categorical("min_samples_split", [2,10,20])
                }
            return params
        ...
        if model_name == "gbmc":
            params = {
                ...
                'min_samples_split': trial.suggest_float('min_sample_split',.1,.5),
                'min_samples_leaf': trial.suggest_float('min_sample_split',.1,.5),
    ...
    def _k1(self,params):
        ...
        if self.comp_type == "regression":
            ...
            model.compile(loss = self.metrics_name , optimizer = adam , metrics = [tf.keras.metrics.MeanSquaredError()] )
        elif self.comp_type == "2class":
            print("this is shape of ytrain", self.ytrain.shape)
    ...
    def obj(self, trial):
        ...
        if self.model_name == "k1":
            ...
            history=model.fit(x= self.xtrain, 
                            y= self.ytrain, 
